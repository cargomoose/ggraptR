...
#   c('x', 'y', XYRangeWidgets(), facetWidgets(),
...
#   c('x', facetWidgets(), if (showXYRangeWgts()) 'xlim',
...
#   c('x', 'y', XYRangeWidgets(), facetWidgets(), if (showAesWgts()) 'fill')
# })
# 
# barWidgets <- reactive({
...
#   c('x', 'y', facetWidgets(), if (showAesWgts()) c('fill'))
# })
# 
# pairsWidgets <- reactive({
#   flog.debug("plotWidgetNames::pairshWidgets()", name='all')
#   c('columns', if (showAesWgts()) c('fill', 'color'))
# })
